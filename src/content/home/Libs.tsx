...
export const Libs: FC<ComponentI18n> = ({ translate, loading }) => (
  <section className="w-full py-28 bg-paper-light">
    <header className="w-full px-4 lg:px-1 m-auto max-w-7xl">
      <Chip className="text-primary bg-primary-light">{translate("section_libs_chip")}</Chip>
      <h2 className="font-bold font-poppins text-3xl lg:text-4xl mt-8">{translate("section_libs_title")}</h2>
      <h3 className="font-poppins max-w-[741px] text-neutral text-lg lg:text-xl mt-4">{translate("section_libs_description")}</h3>
    </header>
    <section className="px-4 lg:px-0 my-12 m-auto max-w-7xl">
          ...
          <section key={lib.name} className="bg-paper flex flex-col gap-3 rounded-lg border border-solid border-neutral-light py-11 px-10">
            <div className="flex flex-row justify-between">
              ...
              <a href={lib.githubLink} target="_blank" className="hidden lg:flex flex-row gap-1 text-primary font-medium focus:outline-dashed">
                {translate("section_libs_button")} <span className="sr-only"> - {lib.name}</span> <GitHubIcon />
              </a>
            </div>
            <p className="text-neutral">{translate(lib.description)}</p>
      ...
      <div className="w-fit m-auto mt-12 lg:mt-24">
        <ButtonLink external to={AppLinks.github} outline>
          {translate("section_libs_github_button")} <GitHubIcon className="ml-1" />
        </ButtonLink>
      </div>
    </section>
  </section>
