...
export const Libs: FC<ComponentI18n> = ({ translate, loading }) => (
  <section className="w-full py-28 bg-paper-light">
    <header className="w-full px-4 lg:px-1 m-auto max-w-7xl">
      <Chip className="text-primary bg-primary-light">{loading ? <Skeleton width="120px" height="10px" /> : translate("section_libs_chip")}</Chip>
      <h2 className="font-bold font-poppins text-3xl lg:text-4xl mt-8">
        {loading ? <Skeleton className="w-full max-w-[200px]" /> : translate("section_libs_title")}
      </h2>
      <h3 className="font-poppins max-w-[741px] text-neutral text-lg lg:text-xl mt-4">
        {loading ? <Skeleton className="w-full" count={3} /> : translate("section_libs_description")}
      </h3>
    </header>
    <section className="px-4 lg:px-0 my-12 m-auto max-w-7xl">
      ...
      <div className="w-fit m-auto mt-12 lg:mt-24">
        <ButtonLink external to={AppLinks.github} outline>
          {loading ? <Skeleton width="120px" /> : translate("section_libs_github_button")} <GitHubIcon className="ml-1" />
        </ButtonLink>
      </div>
    </section>
  </section>
