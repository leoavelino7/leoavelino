    ...
    <div className="relative z-10 px-4 lg:px-1 m-auto max-w-full lg:max-w-7xl">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-y-10">
        <div className="col-span-1 flex flex-col justify-center items-start gap-3">
          <Chip>{loading ? <Skeleton width="100px" height="10px" /> : translate("main_banner_chip")}</Chip>
          ...
          <div className="flex w-full max-w-[513px] mt-1 mb-4">
            <p className="w-full text-neutral text-md md:text-lg leading-snug font-poppins font-medium">
              {loading ? <Skeleton width="100%" count={5} /> : translate("main_banner_description")}
            </p>
          </div>

          <ButtonLink to={AppLinks.homeCategories(language, translate("section_category_id"))}>
            {loading ? <Skeleton width="120px" /> : translate("main_banner_button")} <ArrowDownIcon className="ml-1" />
          </ButtonLink>
        </div>
